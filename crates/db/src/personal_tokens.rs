...
pub async fn delete(
    ...
    let result = sqlx::query(
        "DELETE FROM personal_tokens WHERE id = ? AND user_id = ?",
    )
    .bind(id)
    .bind(user_id)
    .execute(pool)
...
pub async fn find_by_id(
    pool: &SqlitePool,
    id: &str,
) -> Result<Option<PersonalTokenRow>, DbError> {
    sqlx::query_as::<_, PersonalTokenRow>(
        "SELECT id, user_id, name, scope, last_used_at, expires_at, created_at \
...
pub async fn touch_last_used(pool: &SqlitePool, id: &str) -> Result<(), DbError> {
    sqlx::query(
        "UPDATE personal_tokens SET last_used_at = datetime('now') WHERE id = ?",
    )
    .bind(id)
    .execute(pool)
    .await
...
mod tests {
    ...
    async fn create_with_expires_at() {
        let pool = test_pool().await;
        let user_id = create_test_user(&pool).await;
        let row = create(&pool, "tok-2", &user_id, "Expiring", "calendar", Some("2030-01-01T00:00:00Z"))
            .await
            .unwrap();
    ...
    async fn list_by_user_returns_owned_tokens() {
        let pool = test_pool().await;
        let user_id = create_test_user(&pool).await;
        create(&pool, "t1", &user_id, "Token A", "full", None).await.unwrap();
        create(&pool, "t2", &user_id, "Token B", "calendar", None).await.unwrap();
    ...
    async fn list_by_user_excludes_other_users() {
        ...
        create(&pool, "t1", &user1, "Token", "full", None).await.unwrap();
    ...
    async fn delete_removes_token_returns_true() {
        let pool = test_pool().await;
        let user_id = create_test_user(&pool).await;
        create(&pool, "tok-1", &user_id, "Token", "full", None).await.unwrap();
    ...
    async fn delete_wrong_user_returns_false_and_leaves_row() {
        ...
        let user1 = create_test_user(&pool).await;
        let user2 = create_test_user(&pool).await;
        create(&pool, "tok-1", &user1, "Token", "full", None).await.unwrap();
