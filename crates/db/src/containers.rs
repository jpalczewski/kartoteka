...
pub async fn list_all(pool: &SqlitePool, user_id: &str) -> Result<Vec<ContainerRow>, DbError> {
    let rows = sqlx::query_as(
        "SELECT * FROM containers WHERE user_id = ? ORDER BY position ASC",
    )
    .bind(user_id)
    .fetch_all(pool)
    .await?;
...
mod tests {
    ...
    async fn update_patches_fields() {
        ...
        let created = insert(&pool, &uid, &make_req("Original"), 0)
            .await
            .unwrap();
    ...
    async fn pinned_returns_only_pinned() {
        ...
        let created = insert(&pool, &uid, &make_req("Unpinned"), 0)
            .await
            .unwrap();
    ...
    async fn toggle_pin_flips_state() {
        ...
        let after_first = toggle_pin(&pool, &created.id, &uid)
            .await
            .unwrap()
            .unwrap();
        assert!(after_first.pinned);

        let after_second = toggle_pin(&pool, &created.id, &uid)
            .await
            .unwrap()
            .unwrap();
    ...
    async fn progress_counts_lists_and_items() {
        ...
        let container = insert(&pool, &uid, &make_req("Project"), 0)
            .await
            .unwrap();
