...
mod tests {
    ...
    async fn create_comment_on_owned_list() {
        ...
        let comment = create(&pool, &user_id, "list", &list_id, "Great list!", "user", None)
            .await
            .unwrap();
    ...
    async fn create_comment_forbidden_on_other_users_list() {
        ...
        let result = create(&pool, &attacker_id, "list", &list_id, "injected!", "user", None).await;
    ...
    async fn list_comments_returns_all_for_entity() {
        ...
        create(&pool, &user_id, "list", &list_id, "first", "user", None).await.unwrap();
        create(&pool, &user_id, "list", &list_id, "second", "user", None).await.unwrap();

        let comments = list_for_entity(&pool, &user_id, "list", &list_id).await.unwrap();
