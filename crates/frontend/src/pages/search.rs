...
use crate::state::search_route::{
    CompletionFilter, SearchRouteState, search_href, search_state_from_query_map,
};
...
pub fn SearchPage() -> impl IntoView {
    ...
    let search_items = RwSignal::new(Vec::<SearchItemResult>::new());
    ...
    Effect::new(move |_| {
        ...
        leptos::task::spawn_local(async move {
            match api::search_items_page(&client, &state).await {
                Ok(page) => {
                    search_items.set(page.items);
    ...
    let on_load_more = move |_| {
        ...
        leptos::task::spawn_local(async move {
            match api::fetch_next_page::<SearchItemResult>(&client, &cursor).await {
                Ok(page) => {
                    search_items.update(|items| items.extend(page.items));
                    next_cursor.set(page.next_cursor);
                    search_error.set(None);
                }
                Err(error) => {
    ...
    view! {
        <div class="container mx-auto max-w-5xl p-4">
            ...
            {move || {
                ...
                if search_loading.get() && search_items.get().is_empty() {
                    return view! { <LoadingSpinner/> }.into_any();
                }

                let items = search_items.get();
                if items.is_empty() {
                    return view! {
                        <div class="card bg-base-200 border border-base-300">
                            <div class="card-body text-base-content/60">
